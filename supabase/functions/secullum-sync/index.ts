...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Iniciando sync Secullum tipo=${body.tipo} periodo=${dataInicio} a ${dataFim}`);
    ...
    /** Map de cpf → collaborator row (para uso posterior) */
    const colabMap = new Map<string, { id: string; origem: string; foto_url: string | null; secullum_data_alteracao: string | null }>();
    ...
    for (const func of funcionarios) {
      ...
      if (existing && existing.origem === 'MANUAL') {
        ...
        colabMap.set(cpfLimpo, existing);
        continue;
      }

      const status = func.Demissao ? 'desligado' : 'ativo';
      ...
      if (existing && existing.origem === 'SECULLUM') {
        ...
        if (error) {
          ...
          colabMap.set(cpfLimpo, inserted);
        }
      }
    }
    ...
    for (const func of funcionarios) {
      ...
      if (
        colab.foto_url &&
        colab.secullum_data_alteracao === func.DataAlteracao
      ) {
        continue;
      }
    ...
    // ─── Step 6: Sync Calculos ───────────────────────────────────────────────
    console.log('Sincronizando calculos...');

    // Filtrar colaboradores ativos com origem SECULLUM
    const colaboradoresAtivos: Array<{ id: string; cpf: string }> = [];
    for (const [cpf, colab] of colabMap.entries()) {
      if (colab.origem !== 'SECULLUM') continue;

      // Se colaboradorIds foi informado, filtrar
      if (body.colaboradorIds && body.colaboradorIds.length > 0) {
        if (!body.colaboradorIds.includes(colab.id)) continue;
      }
      ...
      if (colabRow?.status === 'ativo') {
        colaboradoresAtivos.push({ id: colab.id, cpf });
      }
    }
    ...
    /** Set de nomes de colunas mapeadas + batidas + primeira coluna (Data) */
        ...
        for (const linha of calcResponse.Linhas) {
          ...
          for (const [colName, idx] of Object.entries(colIndex)) {
            ...
            if (colName === 'Data' || idx === 0) {
              // Primeira coluna - ignorar no mapeamento
              continue;
            }

            if (batidasPattern.test(colName)) {
              // Colunas de entrada/saida
              const key = colName
                .toLowerCase()
                .replace(/\s+/g, '')
                .replace('saída', 'saida')
          ...
          // ─── Step 7: Auto-create apontamento_dia ─────────────────────────
          ...
          // ─── Step 8: Auto-apontamento dias nao-produtivos (RN-12) ────────
          ...
          if (tipoDia === 'FERIAS' && cargaHoraria > 0) {
            ...
            if (!existingFerias) {
              // Criar apontamento_dia para ferias
              ...
              if (!apDiaErr && apDia) {
                // Criar apontamento_item vinculado ao projeto FERIAS
                await supabase
          ...
          if (tipoDia === 'ATESTADO' && cargaHoraria > 0) {
            ...
            if (!existingAtestado) {
              // Criar apontamento_dia para atestado
              const { data: apDia, error: apDiaErr } = await supabase
                .from('apontamento_dia')
                .insert({
                  colaborador_id: colab.id,
              ...
              if (!apDiaErr && apDia) {
                // Criar apontamento_item vinculado ao projeto AFASTAMENTO
                await supabase
                  .from('apontamento_item')
                  .insert({
                    apontamento_dia_id: apDia.id,
    ...
    console.log(`Sincronizacao concluida em ${duracaoMs}ms`);
